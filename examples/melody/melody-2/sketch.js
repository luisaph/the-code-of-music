window.registerP5Sketch((p) => {
  ...
  `;
    ...
    p.draw = () => {
        ...
        cy = p.height/2;
        cx = p.width/4-40;
        p.image(ear, cx+p.width-ear.width-240, p.height/3, 80, ear.height*(80)/ear.width);
        p.push()
            p.textSize(12);
            let textString = '';
            ...
            for(let i=0;i<circles;i++) {
                //draw circle
                let r = (20 + 9*i + amp*(p.sin((p.frameCount)*(2*p.PI*freq*0.002) + (p.PI/4.5*i))))
                p.push()
                    p.noFill();
                    p.strokeWeight(0.6);
        ...
        p.image(mouth, cx-75, p.height/3-25, 100, mouth.height*(100)/mouth.width);
        p.push()
            p.textSize(15);
            p.textStyle(p.BOLD)
            /*if(freq) {
